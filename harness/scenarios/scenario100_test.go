...
// Scenario 100 — fragmented BEEF under loss: fragment-level NACK recovery
//
...
func TestScenario100_BeefFragmentLossRecovery(t *testing.T) {
	ctx := context.Background()
	e, _ := retryTopology(t, "s100")
	e.PatchEnv("s100-proxy", map[string]string{
		"TCP_LISTEN_PORT": "9002",
		"FRAG_MTU":        "1280",
	})
	for _, l := range []string{"s100-listener1", "s100-listener2", "s100-listener3"} {
	...
	e.PatchEnv("s100-retry1", map[string]string{"BEEF_ENABLED": "true"})
	e.StartAll(ctx)
	...
	beforeR := e.Snapshot(ctx, "s100-retry1")
	...
	startGenerator(t, ctx, "s100", []string{
		"beef-gen", "-addr", "[fd10::2]:9002", "-topics", "tm_s100",
	...
	afterR := metrics.ScrapeOrFail(t, e.MetricsURL(ctx, "s100-retry1"))

	gaps := sumListenerDelta("s100", "bsl_gaps_detected_total", beforeL, afterL)
	nacks := sumListenerDelta("s100", "bsl_nacks_dispatched_total", beforeL, afterL)
	completed := sumListenerDelta("s100", "bsl_reassembly_completed_total", beforeL, afterL)
	mismatch := sumListenerDelta("s100", "bsl_reassembly_hash_mismatch_total", beforeL, afterL)
